  ...
  <BODY>
	<TABLE border="0" cellspacing="0" cellpadding="0" height="100%" width="100%">
	  <tr>
	    ...
	    <span class="clsSmallLogoCell"><xsl:value-of select="vsapp/@name"/></span></td>
	  </tr>
			...
			if(xmlhttp.responseXML.xml != ''){
				...
				if(oTmpXML2.parseError.reason==""){
					...
					try{
						var oAddin;
						var oAddinObj;
						var sHostTgt = 	oTmpXML2.selectSingleNode("//userdata//basetgt//text()").xml;
						
						oAddin = window.external.AddIns.Item("WebAppProvider.Connect");
						oAddin.Connected = true;
						oAddinObj = oAddin.Object;
						oAddinObj.AddProvider(document.title, document.title, sHostTgt);
					}catch(e){
						//alert(e.message);
					}
				}else{
...
<xsl:template match="section">
	<span class="homepageinactive"><xsl:value-of select="text()"/></span>
</xsl:template>
	...
	<table class="bugbugspc" cellspacing="0" cellpadding="0" border="0">
		<tr><td VALIGN="bottom" height="0">
			...
			<xsl:apply-templates>
				...
				<xsl:with-param name="tempstr"><xsl:value-of select="@id"/></xsl:with-param>
			</xsl:apply-templates>

				<td class="clsTabTblCellFillTab" height="0"><br/></td>
			</tr></table>
		</td></tr>
		<tr><td class="clsTabTblDispCell" valign="top" height="100%">
			<div>
	...
	<tr align="left">
		<td valign="top" width="250">
			...
			<form id="fileUplForm1" target="hiddenIfram2" enctype="multipart/form-data" method="post">
				<xsl:attribute name="action"><xsl:value-of select="//userdata/filetgt1/text()"/></xsl:attribute>
				...
				<input name="userName" type="hidden"><xsl:attribute name="value"><xsl:value-of select="//userdata/name/text()"/></xsl:attribute></input>
				<input name="userPass" type="hidden"><xsl:attribute name="value"><xsl:value-of select="//userdata/pass/text()"/></xsl:attribute></input>
	...
	<tr align="left">
		<td valign="top" width="250">
			...
			<form id="fileUplForm2" target="hiddenIfram2" enctype="multipart/form-data" method="post">
				<xsl:attribute name="action"><xsl:value-of select="//userdata/filetgt2/text()"/></xsl:attribute>
...
<xsl:template match="textfield">
	<tr><td class="homepageinactive"><xsl:value-of select="@name"/>:</td>
	    <td><input id="vsInstallField" type="text"><xsl:attribute name="name"><xsl:value-of select="@name"/></xsl:attribute></input></td>
	</tr>
</xsl:template>

<xsl:template match="securefield">
	<tr><td class="homepageinactive"><xsl:value-of select="@name"/>:</td>
	    <td><input id="vsInstallField" type="password"><xsl:attribute name="name"><xsl:value-of select="@name"/></xsl:attribute></input></td>
	</tr>
</xsl:template>
