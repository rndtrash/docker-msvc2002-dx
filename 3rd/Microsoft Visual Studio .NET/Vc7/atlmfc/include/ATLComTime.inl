...
ATLCOMTIME_INLINE COleDateTime &COleDateTime::operator=(const SYSTEMTIME &systimeSrc)
{
	...
	// to VariantTimeToSystemTime.
	SYSTEMTIME systimeValidate;
	::VariantTimeToSystemTime(m_dt, &systimeValidate);
	m_status = (systimeValidate.wDay == systimeSrc.wDay)?  valid : invalid;
	...
	int nHour, int nMin, int nSec)
{
	...
	SYSTEMTIME systimeValidate;
	::VariantTimeToSystemTime(m_dt, &systimeValidate);
	return m_status = (systimeValidate.wDay == st.wDay)?  valid : invalid;
}
