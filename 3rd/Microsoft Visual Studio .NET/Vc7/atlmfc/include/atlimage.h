...
inline void CImage::CInitGDIPlus::ReleaseGDIPlus() throw()
{
	...
	if( m_dwToken != 0 )
	{
		if (_AtlBaseModule.m_eDllInitializationStage != Dll_Process_detached)
			Gdiplus::GdiplusShutdown( m_dwToken );
		else
		{
			// Calling GdiplusShutdown after WinMain(DLL_PROCESS_DETACH) will result in a deadlock.
			// Not calling GdiplusShutdown in this case results in a resource leak and will lead to problems later on.
			// Therefore all dynamically created CImage objects must be destructed before DLL_PROCESS_DETACH
			// and there should be no global CImage variables in a DLL.
			ATLASSERT (0);
		}
	}
	m_dwToken = 0;
