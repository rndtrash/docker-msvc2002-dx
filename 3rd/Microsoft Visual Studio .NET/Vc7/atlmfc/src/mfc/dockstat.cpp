...
CDockState::CDockState()
{
	...
	if ((vi.dwMajorVersion > 4) || (vi.dwMajorVersion == 4 && vi.dwMinorVersion != 0))
	{
		...
		m_rectClip.right = GetSystemMetrics(SM_CXVIRTUALSCREEN) - (-m_rectClip.left);
		m_rectClip.bottom = GetSystemMetrics(SM_CYVIRTUALSCREEN) - (-m_rectClip.top);
	}
	else
		m_rectClip = m_rectDevice;

	m_rectClip.left += GetSystemMetrics(SM_CXFRAME);
	m_rectClip.right -= GetSystemMetrics(SM_CXSMICON);
	m_rectClip.top += GetSystemMetrics(SM_CYSMCAPTION) + GetSystemMetrics(SM_CYFRAME);
	m_rectClip.bottom -= GetSystemMetrics(SM_CYSMICON) + GetSystemMetrics(SM_CYFRAME);
}
...
void CDockState::ScalePoint(CPoint& pt)
{
	...
	if (pt.x > m_rectClip.right)
		pt.x = m_rectClip.right - (m_rectDevice.right / 4);
	else if (pt.x < m_rectClip.left)
		pt.x = m_rectClip.left;
	if (pt.y > m_rectClip.bottom)
		pt.y = m_rectClip.bottom;
	else if (pt.y < m_rectClip.top)
		pt.y = m_rectClip.top;
	...
	if ((vi.dwMajorVersion > 4) || (vi.dwMajorVersion == 4 && vi.dwMinorVersion != 0))
	{
		...
		if (hUser != NULL)
		{
			...
			if (pfnMonitorFromPoint && !(pfnMonitorFromPoint)(pt, MONITOR_DEFAULTTONULL))
			{
				pt.x = GetSystemMetrics(SM_CYFRAME);
				pt.y = GetSystemMetrics(SM_CYSMCAPTION) + GetSystemMetrics(SM_CYFRAME);
			}
		}
	}
}
