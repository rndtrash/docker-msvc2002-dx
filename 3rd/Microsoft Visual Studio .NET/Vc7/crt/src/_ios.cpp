...
ios::ios()
{
    ...
    _mtlockinit(lockptr());
    if (InterlockedIncrement((long*)(&fLockcInit)) == 1)
        {
        _mtlockinit(&x_lockc);
        }
#endif  /* _MT */
...
#ifdef _MT
    LockFlg = -1;               // default is now : locking
        if (!InterlockedDecrement((long*)(&fLockcInit)))
        {
        _mtlockterm(&x_lockc);
        }
    _mtlockterm(lockptr());
...
// Initialize this early in the initialization process
// Keep the refcount fLockcInit above 0 in multithreading code
#pragma warning (disable: 4073)
#pragma init_seg( lib )
static class __InitLock : ios {} __initlock;
